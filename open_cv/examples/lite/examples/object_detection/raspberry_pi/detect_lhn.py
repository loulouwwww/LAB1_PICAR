...
def run(model: str, camera_id: int, width: int, height: int, num_threads: int,
    ...
    while cap.isOpened():
        ...
        #contol the car
        new_stop = control_car(_stop_,detections)
        #
        if (new_stop ==0):
            fc.forward(10)
            #time.sleep(1)
...
if __name__ == '__main__':
   main()
    ...
    #     label_allow_list=_ALLOW_LIST,
    #     label_deny_list=_DENY_LIST)
    # detector = ObjectDetector(
    #     model_path=_MODEL_FILE, options=options)

    # image = cv2.imread(_IMAGE_FILE)
    # result = detector.detect(image)
    # print(result)
    # with open(_GROUND_TRUTH_FILE, 'w') as f:
